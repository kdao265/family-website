...
function LoadingCard() {
  return (
    <div className="animate-pulse rounded-2xl bg-surface-container-low p-5">
      <div className="mb-4 h-44 rounded-xl bg-surface-container" />
  ...
  useEffect(() => {
    async function loadHome() {
      ...
      const [membersResult, momentsResult] = await Promise.all([
        supabase
          .from('family_members')
          .select('id, full_name, birth_date, relation_title, short_bio, hobbies, avatar_url')
    ...
    <main className="pt-[72px]">
        ...
        <div className="mx-auto grid max-w-container-max gap-10 lg:grid-cols-[1.25fr_.75fr] lg:items-center">
          <div>
            ...
            <div className="mt-8 flex flex-wrap gap-4">
              <Link
                to="/guestbook"
                className="inline-flex items-center gap-2 rounded-full bg-on-primary px-6 py-3 font-label-md text-primary transition-transform hover:-translate-y-0.5 shadow-sm"
              >
                Xem lời chúc dành cho bà <ArrowRight className="h-4 w-4" />
              </Link>
              <Link
                to="/family-tree"
                className="inline-flex items-center gap-2 rounded-full border border-on-primary/50 px-6 py-3 font-label-md text-on-primary hover:bg-on-primary/10 transition-colors"
              >
                Khám phá gia phả
              </Link>
            </div>
          ...
          <aside className="rounded-3xl border border-on-primary/25 bg-on-primary/10 p-7 backdrop-blur-sm">
            ...
            <Link
              to="/guestbook"
              className="mt-6 inline-flex items-center gap-2 font-label-md underline underline-offset-4 hover:text-on-primary"
            >
              Gửi một lời chúc <ArrowRight className="h-4 w-4" />
            </Link>
          </aside>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-container-max px-margin-mobile py-section-gap md:px-margin-desktop">
        <div className="mb-10 flex flex-wrap items-end justify-between gap-4">
          ...
          >
            Xem gia phả <ArrowRight className="h-4 w-4" />
          </Link>
        </div>
        ...
        {loadError && (
          <div
            role="status"
            className="mb-6 rounded-2xl border border-primary/20 bg-primary/10 px-5 py-4 font-body-md text-on-surface"
          >
            Một phần dữ liệu chưa tải được. Bạn có thể thử tải lại trang sau ít phút.
          </div>
        )}
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {loading
            ...
            : members.map((member) => (
                <article
                  key={member.id}
                  className="overflow-hidden rounded-2xl border border-outline/10 bg-surface-container-lowest family-card-shadow flex flex-col justify-between"
                >
                  <div>
                    {member.avatar_url ? (
                      <img
                      ...
                      <div className="flex h-52 items-center justify-center bg-surface-container-low text-primary">
                    ...
                    <div className="p-5">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        {member.relation_title && (
                          <span className="px-2.5 py-0.5 rounded-full bg-primary/10 text-primary text-xs font-medium shrink-0">
                            {member.relation_title}
                          </span>
                        )}
                      </div>

                      <p className="mt-2 flex items-center gap-2 font-body-md text-sm text-on-surface-variant">
                        <CalendarDays className="h-4 w-4 text-primary shrink-0" />
                        Sinh ngày {formatDate(member.birth_date)}
                      </p>

                      <p className="mt-2 font-body-md text-sm text-on-surface-variant line-clamp-2">
                        {member.hobbies?.join(' · ') || 'Những niềm vui giản dị bên gia đình'}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="p-5 pt-0">
                    <button
                      ...
                      className="font-label-md text-primary hover:underline inline-flex items-center gap-1.5"
                    >
                      Xem vài dòng kỷ niệm <ArrowRight className="h-3.5 w-3.5" />
                    </button>
                  </div>
                </article>
              ))}
        ...
        {!loading && members.length === 0 && (
          <p className="rounded-2xl bg-surface-container-low p-6 font-body-md text-on-surface-variant">
            Thông tin thành viên sẽ được cập nhật trong thời gian tới.
          </p>
        )}
      </section>
      ...
      <section className="border-y border-outline/10 bg-surface-container-low px-margin-mobile py-section-gap md:px-margin-desktop">
        <div className="mx-auto max-w-container-max">
          <div className="mb-10 flex items-end justify-between gap-4">
            <div>
              <p className="font-label-md text-sm text-primary uppercase tracking-wider">Ký ức</p>
              <h2 className="mt-2 font-headline-lg text-headline-lg text-secondary">
                Khoảnh khắc đáng nhớ
              </h2>
            </div>
            <Link
              ...
              className="inline-flex items-center gap-2 font-label-md text-primary hover:underline"
            >
              Xem tất cả <Images className="h-4 w-4" />
            </Link>
          </div>
            ...
            {loading
              ...
              : featuredMoments.map((moment) => (
                  <Link
                    key={moment.id}
                    to="/moments"
                    className="group overflow-hidden rounded-2xl bg-surface shadow-sm hover:shadow-md transition-shadow"
                  >
                    <img
                    ...
                    />
                    <div className="p-5">
                      <p className="font-label-md text-xs text-primary uppercase">
                        {moment.category || 'Kỷ niệm gia đình'}
                      </p>
                      <h3 className="mt-1 font-headline-md text-xl text-on-surface">
                        {moment.title}
                      </h3>
                    </div>
                  </Link>
          ...
          {!loading && featuredMoments.length === 0 && (
            <p className="rounded-2xl bg-surface p-6 font-body-md text-on-surface-variant">
              Album ảnh đang chờ những khoảnh khắc đầu tiên của gia đình.
            </p>
          )}
        </div>
      </section>
      ...
      {/* Lời chúc */}
      <section className="mx-auto max-w-4xl px-margin-mobile py-section-gap text-center md:px-margin-desktop">
        <Heart className="mx-auto h-10 w-10 fill-primary text-primary" />
        <h2 className="mt-5 font-headline-lg text-headline-lg text-secondary">
        ...
        <Link
          to="/guestbook"
          className="mt-7 inline-flex items-center gap-2 rounded-full bg-primary px-6 py-3 font-label-md text-on-primary hover:bg-primary-container transition-colors shadow-sm"
        >
          Viết lời chúc <ArrowRight className="h-4 w-4" />
        </Link>
      </section>
      ...
      {selectedMember && (
        <div
          className="fixed inset-0 z-[60] flex items-center justify-center bg-on-surface/50 p-4 backdrop-blur-xs"
          role="dialog"
          aria-modal="true"
        ...
        >
          <div
            className="relative w-full max-w-md rounded-3xl bg-surface p-7 shadow-2xl border border-outline/10"
          ...
          >
            ...
            {selectedMember.avatar_url && (
              <img
                ...
                className="mb-5 h-52 w-full rounded-2xl object-cover shadow-sm"
              />
            )}

            <div className="flex items-center gap-2 mb-1">
              <span className="font-label-md text-xs uppercase tracking-wider text-primary font-semibold">
                {selectedMember.relation_title || 'MẢNH GHÉP GIA ĐÌNH'}
              </span>
            </div>
            ...
            {selectedMember.short_bio ? (
              <blockquote className="mt-4 rounded-2xl bg-surface-container-low p-4 font-body-md text-sm italic text-on-surface leading-relaxed border-l-4 border-primary">
                "{selectedMember.short_bio}"
              </blockquote>
            ) : (
            ...
            {selectedMember.hobbies && selectedMember.hobbies.length > 0 && (
              <div className="mt-4 pt-4 border-t border-outline/10">
                <p className="font-label-md text-xs text-on-surface-variant mb-2">Sở thích & Niềm vui:</p>
                <div className="flex flex-wrap gap-1.5">
                  {selectedMember.hobbies.map((hobby) => (
                    <span
                      key={hobby}
                      className="rounded-full bg-surface-container-low px-3 py-1 text-xs text-secondary font-medium"
                    >
                      {hobby}
                    </span>
                  ))}
